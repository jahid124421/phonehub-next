...
import { getAllProducts, getScoreForProduct, type Product as JsonProduct } from '@/lib/data';
...
interface SearchProduct {
  ...
  pros: string[];
  cons: string[];
  // Attached server-side so PhoneCard never imports @/lib/data client-side.
...
function jsonFallback(
  ...
  if (ids) {
    ...
    return {
      results: products.map((p) => ({ ...p, score: getScoreForProduct(p.id) })),
...
export async function GET(request: NextRequest) {
  try {
    ...
    const rows = await prisma.$queryRawUnsafe<SearchProduct[]>(
      dataSql,
      ...values,
      limit,
      offset
    );
    const results = rows.map((r) => ({ ...r, score: getScoreForProduct(r.id) }));
